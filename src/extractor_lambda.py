...
def conn_db():
    ...
    try:
        secret_value = sm.get_secret_value(
            SecretId="totesys_creds")['SecretString']
    except sm.exceptions.ResourceNotFoundException:
        return 'ERROR: Secrets Manager can''t \
            find the specified Totesys secret.'
...
def db_tables_fetcher():
    """This PSQL query retrieves all public tables
    ...
    if s3_setup_success(input_bucket):
        if os.path.exists('./tmp'):
            ...
            if len(csv_files) > 0:
                ...
                raise ValueError(
                    'ERROR: No CSV files to upload to S3 are found')
        else:
            raise ValueError('ERROR: can'
                             't locate temp directory to read CSV files!')
    else:
...
def update_csv_export_file(bucket):
    """
      function downloads the Run number in the csv_export.txt
      is ammended to increment the count,#
